...
export class OrderComponent implements OnInit, OnDestroy {
  ...
  getOrders(): void {
    if (
      this.showMyOrdersOnly &&
      this.userId !== null &&
      this.userId === this.currentUser.userId
    ) {
      this.orderService.myOrders(this.userId).subscribe((orders) => {
        this.orders = orders;
        this.dataSource = new MatTableDataSource(this.orders);
      });
    } else {
  ...
  addComment(orderId: number) {
    ...
    if (commentControl) {
      ...
      this.commentService
        .addCommentToOrder(orderId, comment)
        .subscribe((newComment) => {
          ...
          this.userService
            .getByID(newComment.userId.toString())
            .subscribe((user) => {
  ...
  getUserFullName(userId: string): string {
    const order = this.orders.find(
      (order) => order.managerId === Number(userId)
    );
    if (order) {
  ...
  isMyOrder(order: any): boolean {
    ...
    if (
      ...
      order.managerId === this.currentUser.userId ||
